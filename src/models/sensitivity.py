...
from models.metrics import logistic, outcome_metrics, trajectory_indices
...
@cache
def _outcome_metric(model, names, outcome, t1, E0, max_steps=SENSITIVITY_MAX_STEPS):
    if outcome not in ("Rt", "Itot"):
        raise ValueError
    _params = _make_params(MODEL_NAMES[model], names)
    def _out(base_params, row):
        params = _params(base_params, row)
        tt, yy = model(params=params, t1=t1, E0=E0, max_steps=max_steps, throw=False)
        if outcome == "Rt":
            return _Rt(tt, yy, params)
...
def _partial_rank_corr_coeff(X, y):
    ...
    return -W[:-1, -1] / np.sqrt(np.clip(np.diag(W) * W[-1, -1], 1e-12, None))[:-1]

def _prcc_fisher_ci(r, n, d, alpha=0.05):
    """
    Confidence interval for a PRCC via the Fisher z-transform (Marino et al. 2008).
...
def _closed_loop_Rt(R_eps, epsilon_w, k, R_crit=1.0):
    return brentq(lambda r: R_eps * (1.0 - epsilon_w * logistic(k * (r - R_crit))) - r, 1e-9, max(10.0, 2.0 * R_eps))
...
def Rt_elasticities(R_0, p, RR_a, RR_p, epsilon_s, epsilon_w, k=10.0, R_crit=1.0):
    ...
    e = {name: S * e for name, e in open_loop.items()}
    e["epsilon_w"] = -(R_eps * epsilon_w * s / r) * S
    e["R_crit"] = (R_crit / r) * L * S
    e["k"] = -(L * S) * (r - R_crit) / r
    e["tau_W"] = 0.0
    e["tau_B"] = 0.0
    return {"Rt": r, "R_eps": R_eps, "loop_gain": L, "sensitivity_S": S, "elasticities": e, "open_loop": open_loop}

def target_Rt_efficacies(R_0, p, RR_a, RR_p, target, k=10.0, R_crit=1.0):
    """Efficacies at the midpoint of the feasible eps_s window."""
    theta = 1.0 - (1.0 - p) / (p * RR_a + (1.0 - p) * (RR_p + 1.0))
    s = min(logistic(k * (target - R_crit)), 1.0 - 1e-12)
    eps_s_of = lambda R_eps: (1.0 - R_eps / R_0) / (1.0 - theta)
    lo = max(eps_s_of(target / (1.0 - s)), 0.0)
    hi = min(eps_s_of(target), 1.0)
    epsilon_s = 0.5 * (lo + hi)
    R_eps = R_0 * (1.0 - epsilon_s * (1.0 - theta))
    epsilon_w = (1.0 - target / R_eps) / s
    return epsilon_s, epsilon_w, (lo, hi)
